...
class CoreFunctionTests(unittest.TestCase):
    ...
    def test_remote_window_chrome_is_separate_from_control_toolbar(self) -> None:
        ...
        self.assertIn("class NativeRemoteTitlebar", host)
        self.assertIn("nativeTitlebar.Show(this)", host)
        self.assertIn("nativeTitlebar.SetOverlayWidth(ClientSize.Width)", host)
        self.assertIn('title: $("remoteName").textContent || "远程桌面"', html)
        self.assertIn('maximized: document.body.classList.contains("window-maximized")', html)
    ...
    def test_native_glass_toolbar_and_fill_mode_do_not_cut_video_holes(self) -> None:
        ...
        host = (root / "packaging" / "ControlWindowHost.cs").read_text(encoding="utf-8")
        native = (root / "native" / "WindowsLANRemoteVideo.cpp").read_text(encoding="utf-8")
        self.assertIn("class NativeRemoteToolbar", host)
        self.assertNotIn("class NativeGlassToolbar", host)
